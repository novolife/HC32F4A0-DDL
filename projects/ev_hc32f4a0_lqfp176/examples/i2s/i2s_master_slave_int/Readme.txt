...
说明：
本样例实现I2S主、从机全双工通信功能，使用内部时钟，音频采样频率为48K，通过两块板
...
标板2（从机）中；
4）先按下目标板1（主机）复位按键K11，再按下目标板2（从机）复位按键K11；
5）按下目标板1（主机）的K2按键，启动全双工数据循环收发，收发完一次数据后对接收到的
数据与发送的数据对比，完全相同则LED_BLUE亮，则功能正常；否则，LED_RED亮。
