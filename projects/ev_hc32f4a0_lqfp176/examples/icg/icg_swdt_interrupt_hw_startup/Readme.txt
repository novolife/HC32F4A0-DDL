...
说明：
本样例设置硬件启动SWDT，并设置约0.82秒触发一次SWDT计数溢出中断，通过按键K10循环切
...
使用步骤
================================================================================
1）打开并重新编译工程，启动IDE的下载和调试功能；
2）全速运行，进入正常模式LED_BLUE闪烁；
3）按下按键K10切换工作状态到睡眠模式，LED_RED闪烁；
4）再次按下按键K10切换工作状态到停止模式，LED_BLUE和LED_RED闪烁；
5）再次按下按键K10切换工作状态到正常模式，LED_BLUE闪烁。
